...
const Events=({data,base_url,taxation})=>{
  ...
  const [orderbooking,setOrderbooking] = useState({travelDate:{start:start,end:end,noofdays:''},travellers:"",firstName:"",lastName:"",contact:"",email:"",carbonproject:[],bookedItenary:[],advance:"",rooms:1,addons:[],accommodationtype:accomodationtypexc});
...
const callTaxapi=async()=>{
  try{
  let res = await adminapi.get("/taxes" , {headers:headers});
  if(res.data){
 ...
 try {
    const res = await adminapi.get(`/event/${name[1]}`, {headers:headers});
