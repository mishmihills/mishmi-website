...
import BookingDetails from "../../components/accomodation/BookingDetails";
...
const Accomodation=({data,base_url , clientId})=>{
  ...
  const [orderbooking,setOrderbooking] = useState({travelDate:{start:'',end:'',noofdays:''},travellers:defaultcapacity,firstName:"",lastName:"",contact:"",email:"",carbonproject:[],bookedItenary:[],advance:"",rooms:'',addons:[],accommodationtype:accomodationtypexc});
...
let carbonval = Math.ceil(data[0].carbonval * noofroomrequired * newdays);
...
var options = {
      ...
      name: process.env.NEXT_PUBLIC_COMPANYNAME,
      currency: data.currency,
      amount: data.amount,
      order_id: data.id,
      description: "Thankyou for your Payment",
      image: `${process.env.NEXT_PUBLIC_BASEURL}/images/logo1.png`,
  ...
  return (
     ...
     <HtmlHead urlx ={router.asPath} images={images} metaData={data && data.length > 0 && data[0] && data[0].metaData ? data[0].metaData : null}/>
       ...
       <>
        ...
        <TripDetails AOS={AOS} base_url={base_url} setToggle={setToggle} toggle={toggle} description={description} totalcf={grandTotalcart().carbonval} nameite={data[0].name} totalpricetag={getPriceString()} locationdetails={getLocationDetails(data[0])} setToggles={setToggles} toggles={toggles} instabooking={instabooking} togglesx={togglesx} setTogglesx = {setTogglesx}/>
        <BeautySection AOS={AOS} base_url={base_url} activitydata = {getActivitylist(data[0].thingstodo)} images={images} otherdetails ={data && data.length > 0 && data[0] && data[0].otherDetails ? data[0].otherDetails : null }/>
       ...
       <BookingDetails setLoading={setLoading} AOS = {AOS} base_url={base_url} paymentdetails={paymentdetails} sendMail={sendMail}/>
        )} 
       <Footer/>
     </>
     )
