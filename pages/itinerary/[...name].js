...
if(process.env.NEXT_PUBLIC_GOOGLETAGMANAGER && process.env.NEXT_PUBLIC_GOOGLETAGMANAGER !== "") {
  ...
  const getPriceString = (persons) => {
    ...
    if (total && noofperson) {
      let vbb = { tag: `${total} for ${noofperson} Persons.`, value: total };
      setPriceTag(vbb);
    }
  }
  ...
  return (
    <>
      {virtualObj && virtualObj.length !== 0 ?
        <>
          <HtmlHead urlx={router.asPath} images={images} metaData={data?.[0]?.itineraryindex?.[0]?.commondetails?.metaData ?? null} />
          ...
          {paymentdetails === "" ?
            <>
              {successpage === "" ?
                ...
                <BookingEnquiry AOS={AOS} base_url={base_url} successpage={successpage} />
              }
            </> :
            <BookingDetail AOS={AOS} base_url={base_url} paymentdetails={paymentdetails} iten={virtualObj._id} sendMail={sendMail} />
          }
          <Footer />
        </> : ''
      }
...
export async function getServerSideProps(context) {
  ...
  const carbonTraceLogin = async () => {
    ...
    try {
      let res = await axios.post(
        ...
        JSON.stringify({ username: process.env.NEXT_PUBLIC_CARONTRACEUSER, password: process.env.NEXT_PUBLIC_CARONTRACEPASSWORD }),
        { headers }
      );
  ...
  try {
    const res = await adminapi.get(`/getmaskitinerary/${name[1]}`, { headers });
