...
const ShortEnquiry = ({ setShowModal , idx , setIsenquried,sendMail }) => {
  ...
  const handleFormSubmit = async(e) => {
    ...
    if (nameError || whatsappError) {
      ...
      const uploaddata = {name:`${formData.name}`,email:'generalenquiry@xyz.com',phone:formData.whatsapp,message:message,tag:'Enquiry',info:formData};
  ...
  return (
    <ModalBackground>
      <ModalContent>
        <CloseButton onClick={() => {setShowModal(false),setIsenquried(true)}}>
          &times;
        </CloseButton>
