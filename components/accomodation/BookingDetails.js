...
const BookingDetails = (props) => {
    ...
    const { AOS, base_url, paymentdetails, sendMail, setLoading } = props;
        ...
        const createinvoice = async () => {
            if (payment_id && order_id) {
                ...
                if (ress.status === 200 || ress.data === 'success') {
                    ...
                    let html = `Dear ${name},`;
                    html += "<div>" + `Thank you for choosing ${process.env.NEXT_PUBLIC_BRANDNAME} for your upcoming stay.` + "</div>";
                    html += "<p>" + "We are pleased to confirm the details of your booking: " + "</p>";
                    html += "<li>" + `Accommodation Type: ${packagess}` + "</li>";
                    html += "<li>" + `Checkin Date: ${checkindate}` + "</li>";
                    html += "<li>" + `Stay Duration: ${tripindays} Night.` + "</li>";
                    html += "<li>" + `Number of Guests: ${nooftravellers} Pax` + "</li>";
                    html += "<p>" + `Please note that the balance of your booking must be paid in full no later than ${checkindate}, if paid in partial.` + "</p>";
                    html += "<p>" + `Should you have any questions or concerns, please do not hesitate to contact us at ${process.env.NEXT_PUBLIC_SUPPORTEMAIL} or ${process.env.NEXT_PUBLIC_PHONE}` + "</p>";
                    html += "<p>" + `We look forward to providing you with an exceptional experience with us!` + "</p>";
                    html += "<p></p><div>" + "Sincerely," + "</div>";
                    html += "<p>" + `${process.env.NEXT_PUBLIC_BRANDNAME} Support` + "</p>";
                    html += "<div>" + `${process.env.NEXT_PUBLIC_SUPPORTEMAIL}` + "</div>";
                    html += "<div>" + `${process.env.NEXT_PUBLIC_PHONE}` + "</div>";
